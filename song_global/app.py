...
# Defining basic visual page settings (icon & title)
...
# Getting the theme (dark or light)
theme = st_theme()
...
if theme and theme['base'] == 'dark':
    theme_color = 'white'
    map_color = 'CartoDB dark_matter'

else:
    theme_color = 'black'
    map_color = 'CartoDB positron'
...
with col2:
    col2.title('Artists by Country', text_alignment='center')

    c3 = col2.container(horizontal_alignment='center', height=800, border=True)

    with c3:
        subcol4, subcol5 = c3.columns([3, 1])

        with subcol4:

            subc3 = subcol4.container(width=700, border=None)
            ...
            with subc3:
                ...
                choropleth = folium.Choropleth(
                    ...
                    bins=[1, 10, 27, 82, 309],
                    legend_name='Quantidade de artistas por país'
                ).add_to(m)

                folium.GeoJson(
                    'data/worldmap.json',
                    style_function=lambda x: {'fillColor': '#ffffff00', 'color': '#ffffff00'},  # Camada invisível
                    tooltip=folium.GeoJsonTooltip(
                        fields=['ADMIN', 'MY_METRIC'],  # Nomes das propriedades no GeoJSON
                        aliases=['Country:', 'Artists:'],  # Nomes amigáveis que aparecerão no mapa
                        localize=True
                    )
                ).add_to(m)
        ...
        with subcol5:
            option = st.selectbox('Select', ['Op1', 'Op2'])
            option_2 = st.selectbox('Select', ['Op3', 'Op4'])
